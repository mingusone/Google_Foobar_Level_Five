...
def factorial(n):
    # Debugging if we were losing precision here. Nope, we weren't. 
    # fact = 1l
...
# All that work :<
# And it wasn't working properly and I found a far better solution (the above) at:
...
def conjugate_class_size(p, n):
    ...
    for a, j in counts.iteritems():
        total_product *= ((a**j) * factorial(j))
    return n_factorial / total_product

# I had an error for the longest time and it turns out that my partition function was wrong.
# But I thought it was a python issue with storing huge numbers so I wrote this to try to multiply
# without multiplying. Surely you can't have precision errors if I add my way to one quadbiltilgazilon?
# It's a cute function. A good momento. Turns out python's Long can hold HUGE numbers. 
# But the reason why I thought this was because you see that s_power_Z = s**exponent_sum down there?
# It changes ever so slightly when you use math.pow(). And numpy's pow basically dips its head in brine
# when you try to call it with that many digits. 

# def custom_exponent(x,y):
#   if y == 0:
#     return 1
#   if y == 1:
#     return x
#   next_x = custom_exponent(x, y-1)
#   total = 0
#   for i in xrange(x):
#     total += next_x
#   return total
...
def solution(h, w, s):
    ...
    h_factorial = factorial(h)
    w_factorial = factorial(w)
    length_of_G = h_factorial * w_factorial

    #I had to do the below because the generators were acting all funky in nested for loops
    h_partitions = [i for i in partitions(h)]
    w_partitions = [i for i in partitions(w)]
...
# The secret sauce we keep referring back to: 
#Orbits = sum( for g in G, (s**Z)*(cl(gh)*cl(gw)) ) / len(G)
...
# Very fun, one of the best challenges of my life. Also, I can't believe how much I've learned while
# doing these problems. 

# Thanks Google.

# PS: I guess this part is like a message in a bottle right? You don't expect anyone to read it but
# perhaps in 100 years when GenAI is online and is combing through all the old repo's, it'll find this
# message and include in its biography on humans titled "Cute little stories from ancient repos."
